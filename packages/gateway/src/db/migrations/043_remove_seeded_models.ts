/**
 * Migration: Remove Seeded Models
 * Removes the hardcoded models from migration 029 since we now discover them from provider APIs.
 * Only removes models that are not currently referenced by routing rules.
 */

import type postgres from 'postgres';

export async function up(sql: postgres.Sql): Promise<void> {
  // List of model IDs that were seeded in migration 029
  ...
  // Delete models that are not referenced by any routing rules or companion overrides
  // This uses a subquery to check for references
  await sql`
    DELETE FROM model_configs
    WHERE model_id = ANY(${seededModelIds})
    AND id NOT IN (
      SELECT DISTINCT model_config_id FROM routing_rules WHERE model_config_id IS NOT NULL
    )
    AND id NOT IN (
      SELECT DISTINCT model_config_id FROM companion_routing_overrides WHERE model_config_id IS NOT NULL
    )
  `;
}

export async function down(sql: postgres.Sql): Promise<void> {
  // Re-run the seed from migration 029 to restore models
  // Get provider IDs
  const providers = await sql<{ id: string; provider: string }[]>`
    SELECT id, provider FROM provider_configs WHERE category = 'text' OR category IS NULL
  `;
    ...
    await sql`
      INSERT INTO model_configs (
        ...
        (${ollamaId}, 'llama3.2', 'Llama 3.2', TRUE,
         131072, 4096, 0, 0,
         '["chat", "streaming"]'::jsonb,
         '{"content_capability": "SFW_ONLY", "is_abliterated": false, "tier": "LOCAL"}'::jsonb),
        ...
        (${ollamaId}, 'qwen2.5:latest', 'Qwen 2.5 7B', TRUE,
         32768, 4096, 0, 0,
         '["chat", "function_calling", "streaming"]'::jsonb,
         '{"content_capability": "SUGGESTIVE", "is_abliterated": false, "tier": "LOCAL"}'::jsonb),
        (${ollamaId}, 'goekdenizguelmez/JOSIEFIED-Qwen3:8b', 'JOSIEFIED Qwen3 8B', TRUE,
         32768, 4096, 0, 0,
         '["chat", "function_calling", "streaming"]'::jsonb,
         '{"content_capability": "UNRESTRICTED", "is_abliterated": true, "tier": "LOCAL"}'::jsonb)
      ON CONFLICT (provider_config_id, model_id) DO NOTHING
    `;
  }
}
