...
class ContextBuilder:
    ...
    def build_system_prompt(
        ...
        if self._companion_generates_images(companion_spec):
            ...
            logger.debug(
                ...
                companion_id=str(companion_spec.id) if hasattr(companion_spec, 'id') else None,
                tools=companion_spec.allowed_tools,
            )
        else:
            logger.debug(
                "image_instruction_skipped",
                companion_id=str(companion_spec.id) if hasattr(companion_spec, 'id') else None,
                tools=companion_spec.allowed_tools,
            )
    ...
    def _companion_generates_images(self, companion_spec: CompanionSpec) -> bool:
        """Check if companion has image-generating tools.

        Used to conditionally include image generation instructions in the prompt,
        saving ~250 tokens for text-only companions.
