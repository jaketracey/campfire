#!/bin/bash
# =============================================================================
# Campfire AWS Infrastructure - Create S3 Buckets (Staging)
# =============================================================================
...
# Block public access (production-like security)
aws s3api put-public-access-block \
    --bucket "${S3_MEDIA_BUCKET}" \
    --public-access-block-configuration \
    "BlockPublicAcls=true,IgnorePublicAcls=true,BlockPublicPolicy=true,RestrictPublicBuckets=true"

# Enable server-side encryption (production-like)
...
# Add CORS configuration for web uploads (restrictive for staging)
aws s3api put-bucket-cors \
    --bucket "${S3_MEDIA_BUCKET}" \
    --cors-configuration '{
        "CORSRules": [
            {
                "AllowedOrigins": ["https://*.campfire.dev", "https://staging.campfire.dev"],
...
# -----------------------------------------------------------------------------
# Create Vault Bucket (Encrypted with KMS)
# -----------------------------------------------------------------------------
...
# Enable server-side encryption with AWS managed KMS keys
aws s3api put-bucket-encryption \
    --bucket "${S3_VAULT_BUCKET}" \
    --server-side-encryption-configuration '{
        "Rules": [
...
# Add lifecycle policy (longer retention for vault)
aws s3api put-bucket-lifecycle-configuration \
    --bucket "${S3_VAULT_BUCKET}" \
    --lifecycle-configuration '{
        "Rules": [
...
aws s3api put-bucket-tagging \
    --bucket "${S3_VAULT_BUCKET}" \
    --tagging "TagSet=[{Key=Project,Value=${TAG_PROJECT}},{Key=Environment,Value=${TAG_ENVIRONMENT}},{Key=Purpose,Value=vault-storage},{Key=Encryption,Value=aws-kms},{Key=CostCenter,Value=${TAG_COST_CENTER}}]"
...
echo "  - Lifecycle: 60d -> IA, 180d -> Glacier IR"
echo ""
echo "Vault Bucket: ${S3_VAULT_BUCKET}"
echo "  - Versioning: Enabled"
echo "  - Encryption: AWS KMS"
echo "  - Lifecycle: 30d -> IA"
echo ""
