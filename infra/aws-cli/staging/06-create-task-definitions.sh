#!/bin/bash
# =============================================================================
# Campfire AWS Infrastructure - Create ECS Task Definitions (Staging)
# =============================================================================
# Creates ECS Fargate task definitions for all services
# with production-like resource allocations
#
...
cat > /tmp/gateway-task-def.json << EOF
{
    ...
    "containerDefinitions": [
        {
            "name": "gateway",
            "image": "${ECR_REGISTRY}/${ECR_GATEWAY_REPO}:latest",
            "essential": true,
            "portMappings": [
                {
                    "containerPort": 4000,
...
cat > /tmp/orchestrator-task-def.json << EOF
{
    ...
    "containerDefinitions": [
        {
            "name": "orchestrator",
            "image": "${ECR_REGISTRY}/${ECR_ORCHESTRATOR_REPO}:latest",
            "essential": true,
            "portMappings": [
                {
                    "containerPort": 5000,
...
cat > /tmp/web-task-def.json << EOF
{
    ...
    "containerDefinitions": [
        {
            "name": "web",
            "image": "${ECR_REGISTRY}/${ECR_WEB_REPO}:latest",
            "essential": true,
            "portMappings": [
                {
                    "containerPort": 3000,
...
cat > /tmp/workers-task-def.json << EOF
{
    ...
    "containerDefinitions": [
        {
            "name": "workers",
            "image": "${ECR_REGISTRY}/${ECR_WORKERS_REPO}:latest",
            "essential": true,
            "portMappings": [
                {
                    "containerPort": 8080,
...
echo ""
echo "============================================================================="
echo "ECS Task Definitions Created Successfully"
echo "============================================================================="
...
echo "  - Init process enabled (for proper signal handling)"
echo "  - Structured logging configuration"
echo "  - Health checks with appropriate timeouts"
echo ""
